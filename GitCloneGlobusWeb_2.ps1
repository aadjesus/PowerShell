...
if ([string]::IsNullOrEmpty($pathDestino) -or [string]::IsNullOrEmpty($token)) {
   Write-Host "Parâmetros obrigatários não informados 'pathDestino' ou 'token'." -ForegroundColor Red
   Exit 1
}
...
try {
	...
	if (-not (Test-Path $pathDestino)) {
		Write-Host "`nCriando diretório: $pathDestino"
		New-Item $pathDestino -Type Directory -Force
	}
	...
	if ($response.value -eq $null -or $lista -eq $null) {
		Write-Host "`nNão foi possível conectar no 'https://dev.azure.com/$organization/$project/' com o token informado, ou nenhum repositório para o prefixo 'GlobusWeb.*'."  -ForegroundColor Red
		return		
	}

	$nomesConcatenados = ($lista.name -join "`n")
	Write-Host "`nRepositórios encontrados com o prefixo 'GlobusWeb.*':`n$nomesConcatenados"
